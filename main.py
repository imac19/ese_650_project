...
#         loss = torch.mean((q1-target_q1)** 2 + (q2 - target_q2)**2)
	...
	def save(self):
		...
		self.critic1.save('critic1_checkpoint.pt')
		self.critic2.save('critic2_checkpoint.pt')

		self.actor_target.save('actor_target_checkpoint.pt')
		self.critic1_target.save('critic1_target_checkpoint.pt')
		self.critic2_target.save('critic2_target_checkpoint.pt')

		torch.save(self.actor_optimizer.state_dict(), 'actor_optimizer_checkpoint.pt')
		torch.save(self.critic1_optimizer.state_dict(), 'critic1_optimizer_checkpoint.pt')
		torch.save(self.critic2_optimizer.state_dict(), 'critic2_optimizer_checkpoint.pt')
...
if __name__ == "__main__": 
	...
	if args.mode == 'train': 
		...
		for i in tqdm.tqdm(range(training_iterations)):
			...
			if done:
				...
				train_episodes += 1
				train_reward = 0
				agent.save(); tqdm.tqdm.write('Saved Model')
	...
	if args.mode == 'test': 
		...
		# Load our best policy 
		policy = Actor(state_dim, action_dim, max_action)
		policy.load('actor_checkpoint.pt')
		policy.to('cpu')
		...
		# save_frames_as_gif(frames, filename=args.env+'.gif')
		plt.figure(1)
		plt.plot(np.arange(iters), r)
		plt.show()
		# 	done = True